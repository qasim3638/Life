...
import { 
  Tag, Search, ArrowLeft, Package, RefreshCw, 
...
export const ClearanceProducts = () => {
  ...
  const fetchProducts = useCallback(async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast.error('Failed to fetch clearance products');
      }
    } catch (error) {
      console.error('Error fetching clearance products:', error);
      toast.error('Failed to load clearance products');
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="p-6 space-y-6" data-testid="clearance-products-page">
      {/* Header */}
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-gray-500">Discontinued and clearance items from suppliers</p>
          </div>
        </div>
        ...
        <Card className="border-amber-200 bg-amber-50">
          <CardContent className="pt-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-amber-700">Total Clearance</p>
                <p className="text-2xl font-bold text-amber-800">{stats.total}</p>
              </div>
              <Tag className="h-8 w-8 text-amber-500/40" />
            </div>
          </CardContent>
        </Card>
          ...
          <div className="flex flex-col md:flex-row gap-4">
            <div className="flex-1 relative">
              ...
              <Input
                ...
                className="pl-10"
                data-testid="clearance-search-input"
              />
            </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-lg flex items-center gap-2">
            ...
            Clearance Items ({products.length})
          </CardTitle>
        </CardHeader>
        <CardContent>
          {loading ? (
            ...
            <div className="text-center py-12 text-muted-foreground">
              ...
              <p className="text-sm mt-1">Add clearance products from the Sync Hub</p>
            </div>
          ) : (
            <div className="space-y-3">
              {products.map((product) => (
                <div 
                  key={product.sku}
                  className="border rounded-lg p-4 bg-amber-50/50 border-amber-200 hover:shadow-md transition-shadow"
                  data-testid={`clearance-product-${product.sku}`}
                >
