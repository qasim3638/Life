...
import { Tag, Search, Grid, List, ChevronDown, X, Filter, Package } from 'lucide-react';
...
const ClearancePage = () => {
  ...
  const fetchProducts = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/api/sync-staging/special-products/clearance?${params}`);
      if (response.ok) {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <SeoHead
        title="Clearance Tiles · Discontinued Lines & End-of-Stock Bargains"
        description="Save big on clearance tiles at Tile Station — discontinued lines, end-of-stock bargains and last-chance deals on premium kitchen, bathroom and floor tiles. Free UK delivery on orders over £500."
        canonical="/clearance"
        keywords="clearance tiles, discounted tiles, end of line tiles, cheap tiles UK, tile sale, discontinued tiles"
      />
      <ShopHeader />
      ...
      <div className="bg-gradient-to-r from-amber-600 to-orange-500 text-white py-12">
        <div className="container mx-auto px-4">
          ...
          <p className="text-xl text-amber-100 max-w-2xl">
            Discontinued lines and end-of-stock items at amazing prices. 
            Limited quantities available - once they're gone, they're gone!
          </p>
          <div className="mt-6 flex items-center gap-4">
            <div className="bg-white/20 backdrop-blur px-4 py-2 rounded-lg">
              <span className="text-2xl font-bold">{stats.total}</span>
              <span className="ml-2 text-amber-100">Products Available</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        {loading ? (
          ...
          <div className="text-center py-20">
            ...
            <h2 className="text-2xl font-semibold text-gray-700 mb-2">No Clearance Items Found</h2>
            <p className="text-gray-500 mb-6">Check back soon for new clearance deals!</p>
            <Link to="/tiles">
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm overflow-hidden hover:shadow-md transition-shadow group">
      <div className="relative aspect-square bg-gray-100">
        ...
        <span className="absolute top-3 left-3 bg-amber-500 text-white text-xs px-2 py-1 rounded-full font-medium">
          CLEARANCE
        </span>
      </div>
      <div className="p-4">
